...
class S3ToHiveTransfer(BaseOperator):
        ...
        **kwargs
        ...
        if self.check_headers and not (self.field_dict is not None and self.headers):
            raise AirflowException(
                "To check_headers provide " + "field_dict and headers"
            )
    ...
    def execute(self, context):
        ...
        with TemporaryDirectory(prefix="tmps32hive_") as tmp_dir, NamedTemporaryFile(
            ...
            self.log.info(
                "Dumping S3 key {0} contents to local file {1}".format(
    ...
    def _match_headers(self, header_list):
        ...
        if not all(test_field_match):
            self.log.warning(
                "Headers do not match field names"
                "File headers:\n {header_list}\n"
                "Field names: \n {field_names}\n".format(**locals())
            )
            return False
