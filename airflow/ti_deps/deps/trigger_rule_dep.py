...
class TriggerRuleDep(BaseTIDep):
    ...
    def _get_dep_statuses(self, ti, session, dep_context):
        ...
        qry = session.query(
            ...
            func.coalesce(
                func.sum(case([(TI.state == State.UPSTREAM_FAILED, 1)], else_=0)), 0
            ),
            ...
            TI.state.in_(
                [State.SUCCESS, State.FAILED, State.UPSTREAM_FAILED, State.SKIPPED]
            ),
        )
        ...
        if tr == TR.ONE_SUCCESS:
            if successes <= 0:
                yield self._failing_status(
                    ...
                    "upstream_tasks_state={1}, upstream_task_ids={2}".format(
                        tr, upstream_tasks_state, task.upstream_task_ids
                    )
                )
        elif tr == TR.ONE_FAILED:
            ...
            if num_failures > 0:
                yield self._failing_status(
                    ...
                    "upstream_tasks_state={2}, upstream_task_ids={3}".format(
                        tr, num_failures, upstream_tasks_state, task.upstream_task_ids
                    )
                )
        elif tr == TR.ALL_FAILED:
            ...
            if num_successes > 0:
                yield self._failing_status(
                    ...
                    "upstream_tasks_state={2}, upstream_task_ids={3}".format(
                        tr, num_successes, upstream_tasks_state, task.upstream_task_ids
                    )
                )
        elif tr == TR.ALL_DONE:
            if not upstream_done:
                yield self._failing_status(
                    ...
                    "upstream_task_ids={3}".format(
                        tr, upstream_done, upstream_tasks_state, task.upstream_task_ids
                    )
                )
        else:
            yield self._failing_status(
