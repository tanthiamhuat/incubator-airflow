...
op_perms = {"can_conf", "can_varimport"}
...
ROLE_CONFIGS = [
    {"role": "Viewer", "perms": viewer_perms, "vms": viewer_vms | dag_vms},
    {
        "role": "User",
...
class AirflowSecurityManager(SecurityManager):
    ...
    def get_user_roles(self, user=None):
        ...
        if user.is_anonymous():
            ...
            return (
                [appbuilder.security_manager.find_role(public_role)]
                if public_role
                else []
            )
        return user.roles
    ...
    def create_custom_dag_permission_view(self):
        ...
        all_dags_models = (
            settings.Session.query(models.DagModel)
        ...
        all_perm_view_by_user = (
            settings.Session.query(ab_perm_view_role)
            .join(
                perm_view, perm_view.id == ab_perm_view_role.columns.permission_view_id
            )
        ...
        for role in dag_role:
            ...
            existing_perm_view_by_user = self.get_session.query(
                ab_perm_view_role
            ...
            for perm_view_id in missing_perm_views:
                update_perm_views.append(
                    {"permission_view_id": perm_view_id, "role_id": role.id}
                )
