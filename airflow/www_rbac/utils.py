...
def generate_pages(current_page,
    ...
    page_link = (void_link
                 if current_page >= num_of_pages - 1 else '?{}'.format(
                     get_params(
                         page=current_page + 1,
                         search=search,
    ...
    output.append(
        last_node.format(
            href_link="?{}".format(
                get_params(
                    page=last_page, search=search, showPaused=showPaused)),
            disabled=is_disabled))
...
def task_instance_link(attr):
    ...
    task_id = bleach.clean(
        attr.get('task_id')) if attr.get('task_id') else None
