...
def should_hide_value_for_key(key_name):
    ...
    if key_name:
        config_set = configuration.conf.getboolean(
            "admin", "hide_sensitive_variable_fields"
        )
        field_comp = any(
            s in key_name.lower() for s in DEFAULT_SENSITIVE_VARIABLE_FIELDS
        )
...
def generate_pages(current_page, num_of_pages, search=None, showPaused=None, window=7):
    ...
    if current_page > 0:
        page_link = "?{}".format(
            get_params(page=(current_page - 1), search=search, showPaused=showPaused)
        )
...
def json_response(obj):
    ...
    """
    return Response(
        response=json.dumps(obj, indent=4, cls=AirflowJsonEncoder),
        status=200,
...
def dag_run_link(attr):
    ...
    execution_date = attr.get("execution_date")
    url = url_for(
        "Airflow.graph", dag_id=dag_id, run_id=run_id, execution_date=execution_date
    )
    return Markup('<a href="{url}">{run_id}</a>'.format(**locals()))
