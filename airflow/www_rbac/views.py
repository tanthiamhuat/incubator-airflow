...
class Airflow(AirflowBaseView):
    ...
    def index(self, session=None):
        ...
        sql_query = session.query(DM).filter(~DM.is_subdag, DM.is_active)
        ...
        if "all_dags" in filter_dag_ids:
            ...
            orm_dags = {
                dag.dag_id: dag
                for dag in sql_query.filter(DM.dag_id.in_(filter_dag_ids)).all()
            }
            unfiltered_webserver_dags = [
                dag for dag in unfiltered_webserver_dags if dag.dag_id in filter_dag_ids
            ]
    ...
    def circles(self):
        return render_template("airflow/circles.html", hostname=socket.getfqdn()), 404
    ...
    def rendered(self):
        ...
        for template_field in task.__class__.template_fields:
            ...
            if template_field in wwwutils.get_attr_renderer():
                html_dict[template_field] = wwwutils.get_attr_renderer()[
                    template_field
                ](content)
            else:
    ...
    def task(self):
        ...
        for attr_name in dir(task):
            if not attr_name.startswith("_"):
                ...
                if (
                    type(attr) != type(self.task)
                    and attr_name not in wwwutils.get_attr_renderer()
                ):  # noqa
        ...
        for attr_name in wwwutils.get_attr_renderer():
            if hasattr(task, attr_name):
                ...
                special_attrs_rendered[attr_name] = wwwutils.get_attr_renderer()[
                    attr_name
                ](source)
...
class DagRunModelView(AirflowModelView):
    ...
    validators_columns = {"dag_id": [validators.DataRequired()]}
    ...
    def action_set_running(self, drs, session=None):
        try:
            ...
            dirty_ids = []
            for dr in (
                session.query(DR).filter(DR.id.in_([dagrun.id for dagrun in drs])).all()
            ):
