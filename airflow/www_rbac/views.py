...
from flask import (g, redirect, request, Markup, Response, render_template,
...
class Airflow(AirflowBaseView):
    ...
    def index(self, session=None):
        ...
        if 'all_dags' in filter_dag_ids:
            ...
            orm_dags = {
                dag.dag_id: dag
                for dag in sql_query.filter(DM.dag_id.in_(filter_dag_ids))
                .all()
            }
            unfiltered_webserver_dags = [
                dag for dag in unfiltered_webserver_dags
    ...
    def task_stats(self, session=None):
        ...
        LastDagRun = (
            ...
            .filter(DagRun.state != State.RUNNING).filter(
                Dag.is_active == True)  # noqa
            .group_by(DagRun.dag_id).subquery('last_dag_run'))
        RunningDagRun = (
    ...
    def blocked(self, session=None):
        ...
        if filter_dag_ids:
            dags = (session.query(DR.dag_id, sqla.func.count(DR.id))
                    .filter(DR.state == State.RUNNING).group_by(DR.dag_id))
            if 'all_dags' not in filter_dag_ids:
            ...
            def clean_column_names():
                ...
                if self.list_columns:
                    self.list_columns = dict(
                        (k.lstrip('_'), v)
                        for k, v in self.list_columns.items())
...
class PoolModelView(AirflowModelView):
    ...
    def fused_slots(attr):
        ...
        if pool_id is not None and used_slots is not None:
            ...
            return Markup(
                "<a href='{url}'>{used_slots}</a>".format(**locals()))
        else:
            return Markup('<span class="label label-danger">Invalid</span>')
    ...
    def fqueued_slots(attr):
        ...
        if pool_id is not None and queued_slots is not None:
            url = '/taskinstance/list/?_flt_3_pool=' + str(pool_id) + \
                  '&_flt_3_state=queued'
            return Markup(
                "<a href='{url}'>{queued_slots}</a>".format(**locals()))
        else:
            return Markup('<span class="label label-danger">Invalid</span>')
