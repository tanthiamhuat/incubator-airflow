...
class WorkerConfiguration(LoggingMixin):
    ...
    def _get_init_containers(self, volume_mounts):
        ...
        init_environment = [
            ...
            {"name": "GIT_SYNC_BRANCH", "value": self.kube_config.git_branch},
            {"name": "GIT_SYNC_ROOT", "value": "/tmp"},
            {"name": "GIT_SYNC_DEST", "value": "dags"},
            {"name": "GIT_SYNC_ONE_TIME", "value": "true"},
        ]
        if self.kube_config.git_user:
            init_environment.append(
                {"name": "GIT_SYNC_USERNAME", "value": self.kube_config.git_user}
            )
        if self.kube_config.git_password:
            init_environment.append(
                {"name": "GIT_SYNC_PASSWORD", "value": self.kube_config.git_password}
            )
    ...
    def _get_environment(self):
        ...
        if (
            ...
            env["AIRFLOW__CORE__SQL_ALCHEMY_CONN"] = conf.get(
    ...
    def init_volumes_and_mounts(self):
        ...
        volumes = [
            _construct_volume(dags_volume_name, self.kube_config.dags_volume_claim),
            _construct_volume(logs_volume_name, self.kube_config.logs_volume_claim),
        ]
        ...
        if self.kube_config.dags_volume_claim:
            ...
            dag_volume_mount_path = os.path.join(
                self.worker_airflow_dags, self.kube_config.git_subpath
            )
        dags_volume_mount = {
