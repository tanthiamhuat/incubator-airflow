...
class DataFlowJavaOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        hook.start_java_dataflow(
            self.task_id, dataflow_options, self.jar, self.job_class
        )
...
class DataflowTemplateOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        hook.start_template_dataflow(
            self.task_id, self.dataflow_default_options, self.parameters, self.template
        )
...
class DataFlowPythonOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        formatted_options = {
            camel_to_snake(key): dataflow_options[key] for key in dataflow_options
        }
        hook.start_python_dataflow(
            self.task_id, formatted_options, self.py_file, self.py_options
        )
