    ...
    try:
        ...
        if e.code in [401, 403, 500]:
            ...
            raise JenkinsException(
                "Error in request. "
                + "Possibly authentication failed [%s]: %s" % (e.code, e.msg)
            )
        elif e.code == 404:
...
class JenkinsJobTriggerOperator(BaseOperator):
    ...
    def build_job(self, jenkins_server):
        ...
        request = Request(
            jenkins_server.build_job_url(self.job_name, self.parameters, None), b""
        )
    ...
    def poll_job_in_queue(self, location, jenkins_server):
        ...
        raise AirflowException(
            "The job hasn't been executed" " after polling the queue %d times",
    ...
    def execute(self, context):
        ...
        if not self.job_name:
            ...
            raise AirflowException(
                "The job_name parameter is missing," "impossible to trigger the job"
            )
