...
class KubeConfig:
    ...
    def _validate(self):
        if not self.dags_volume_claim and (not self.git_repo
                                           or not self.git_branch):
        ...
        while True:
            try:
                self.resource_version = self._run(
                    kube_client, self.resource_version, self.worker_uuid)
            except Exception:
                self.log.exception(
    ...
    def process_status(self, pod_id, status, labels, resource_version):
        if status == 'Pending':
            ...
            self.log.warn(
                ...
                'resource_version: %s', status, pod_id, labels,
                resource_version)
...
class AirflowKubernetesScheduler(LoggingMixin):
    ...
    def _strip_unsafe_kubernetes_special_chars(string):
        ...
        """
        return ''.join(
            ch.lower() for ind, ch in enumerate(string) if ch.isalnum())
    ...
    def _labels_to_key(self, labels):
        try:
            return (labels['dag_id'], labels['task_id'],
                    self._label_safe_datestring_to_datetime(
...
class KubernetesExecutor(BaseExecutor, LoggingMixin):
    def __init__(self):
        ...
        super(KubernetesExecutor,
              self).__init__(parallelism=self.kube_config.parallelism)
    ...
    def clear_not_launched_queued_tasks(self):
        ...
        for State.LAUNCHED
        """
        queued_tasks = self._session.query(TaskInstance).filter(
        ...
        for task in queued_tasks:
            ...
            if len(pod_list.items) == 0:
                ...
                self._session.query(TaskInstance).filter(
                    ...
                    TaskInstance.execution_date == task.execution_date).update(
                        {
        ...
        def _create_or_update_secret(secret_name, secret_path):
            try:
                ...
                self.log.exception(
                    ...
                    'Secret name: %s, error details: %s', secret_name, e)
                raise
    ...
    def start(self):
        ...
        self.kube_scheduler = AirflowKubernetesScheduler(
            ...
            self._session, self.kube_client, self.worker_uuid)
    ...
    def execute_async(self, key, command, queue=None, executor_config=None):
        ...
        kube_executor_config = KubernetesExecutorConfig.from_dict(
            executor_config)
    ...
    def _change_state(self, key, state, pod_id):
        ...
        item = self._session.query(TaskInstance).filter_by(
            dag_id=dag_id, task_id=task_id, execution_date=ex_time).one()
        if state:
            item.state = state
