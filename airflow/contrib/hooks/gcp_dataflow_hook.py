...
class _DataflowJob(LoggingMixin):
    def __init__(
        self, dataflow, project_number, name, location, poll_sleep=10, job_id=None
    ):
    ...
    def _get_job(self):
        if self._job_id:
            job = (
                self._dataflow.projects()
                .jobs()
                .get(projectId=self._project_number, jobId=self._job_id)
                .execute(num_retries=5)
            )
        elif self._job_name:
        ...
        if job and "currentState" in job:
            self.log.info(
                "Google Cloud DataFlow job %s is %s", job["name"], job["currentState"]
            )
        elif job:
            self.log.info(
        ...
        while True:
            if self._job and "currentState" in self._job:
                if "JOB_STATE_DONE" == self._job["currentState"]:
                    ...
                    "JOB_STATE_RUNNING" == self._job["currentState"]
                    and "JOB_TYPE_STREAMING" == self._job["type"]
                ):
                    return True
                elif "JOB_STATE_FAILED" == self._job["currentState"]:
                    raise Exception(
                        "Google Cloud Dataflow job {} has failed.".format(
                            self._job["name"]
                        )
                    )
                elif "JOB_STATE_CANCELLED" == self._job["currentState"]:
                    raise Exception(
                        "Google Cloud Dataflow job {} was cancelled.".format(
                            self._job["name"]
                        )
                    )
...
class _Dataflow(LoggingMixin):
    ...
    def wait_for_done(self):
        ...
        if self._proc.returncode is not 0:
            raise Exception(
                "DataFlow failed with return code {}".format(self._proc.returncode)
            )
        return job_id
...
class DataFlowHook(GoogleCloudBaseHook):
    def __init__(
        self, gcp_conn_id="google_cloud_default", delegate_to=None, poll_sleep=10
    ):
    ...
    def _start_dataflow(
        self, task_id, variables, name, command_prefix, label_formatter
    ):
        variables = self._set_variables(variables)
        cmd = command_prefix + self._build_cmd(task_id, variables, label_formatter)
    ...
    def start_java_dataflow(
        self, task_id, variables, dataflow, job_class=None, append_job_name=True
    ):
        ...
        command_prefix = (
            ["java", "-cp", dataflow, job_class]
            if job_class
            else ["java", "-jar", dataflow]
        )
        self._start_dataflow(task_id, variables, name, command_prefix, label_formatter)

    def start_template_dataflow(
        self, task_id, variables, parameters, dataflow_template, append_job_name=True
    ):
        name = self._build_dataflow_job_name(task_id, append_job_name)
        self._start_template_dataflow(name, variables, parameters, dataflow_template)

    def start_python_dataflow(
        self, task_id, variables, dataflow, py_options, append_job_name=True
    ):
        name = self._build_dataflow_job_name(task_id, append_job_name)
    ...
    def _start_template_dataflow(self, name, variables, parameters, dataflow_template):
        ...
        request = (
            ...
            .templates()
            .launch(
                projectId=variables["project"], gcsPath=dataflow_template, body=body
            )
        )
