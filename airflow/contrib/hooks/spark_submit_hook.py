...
class SparkSubmitHook(BaseHook, LoggingMixin):
    ...
    def __init__(self,
        ...
        if self._is_kubernetes and kube_client is None:
            raise RuntimeError(
                "{master} specified by kubernetes dependencies are not installed!".
                format(self._connection['master']))
    ...
    def _get_spark_binary_path(self):
        ...
        if self._connection['spark_home']:
            connection_cmd = [
                os.path.join(self._connection['spark_home'], 'bin',
    ...
    def _build_spark_submit_command(self, application):
        ...
        if self._conf:
            for key in self._conf:
                connection_cmd += [
                    "--conf", "{}={}".format(key, str(self._conf[key]))
                ]
        ...
        if self._is_kubernetes:
            connection_cmd += [
                "--conf", "spark.kubernetes.namespace={}".format(
        ...
        if self._connection['deploy_mode']:
            connection_cmd += [
                "--deploy-mode", self._connection['deploy_mode']
            ]
    ...
    def _start_driver_status_tracking(self):
        ...
        while self._driver_status not in [
                "FINISHED", "UNKNOWN", "KILLED", "FAILED", "ERROR"
        ]:
            ...
            self._process_spark_status_log(
                iter(status_process.stdout.readline, ''))
