...
class SparkSubmitHook(BaseHook, LoggingMixin):
    ...
    def submit(self, application="", **kwargs):
        ...
        self.log.debug(
            "Should track driver: {}".format(self._should_track_driver_status)
        )
        ...
        for line in itr:
            ...
            # the status using the driver id. Also, we can kill the driver when needed.
                ...
                if match_driver_id:
                    ...
                    self.log.info(
                        "identified spark driver id: {}".format(self._driver_id)
                    )
    ...
    def _start_driver_status_tracking(self):
        ...
        while self._driver_status not in [
            ...
            self.log.debug(
                "polling status of spark driver with id {}".format(self._driver_id)
            )
    ...
    def on_kill(self):
        ...
        if self._submit_sp and self._submit_sp.poll() is None:
            ...
            if self._yarn_application_id:
                self.log.info(
                    "Killing application {} on YARN".format(self._yarn_application_id)
                )
            ...
            if self._kubernetes_driver_pod:
                self.log.info(
                    "Killing pod %s on Kubernetes", self._kubernetes_driver_pod
                )
