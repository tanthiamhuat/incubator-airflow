        ...
        if not self.client:
            ...
            if self.jira_conn_id is not None:
                ...
                if conn.extra is not None:
                    ...
                    # validate
                    if (
                        "validate" in extra_options
                        and extra_options["validate"].lower() == "false"
                    ):
                        validate = False

                    if (
                        "get_server_info" in extra_options
                ...
                try:
                    ...
                    raise AirflowException(
                        "Failed to create jira client, jira error: %s" % str(jira_error)
                    )
                except Exception as e:
                    raise AirflowException(
                        "Failed to create jira client, error: %s" % str(e)
                    )
