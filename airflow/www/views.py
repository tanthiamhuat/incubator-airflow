...
def dag_link(v, c, m, p):
    ...
    url = url_for("airflow.graph", dag_id=dag_id, execution_date=m.execution_date)
...
def label_link(v, c, m, p):
    try:
        ...
        default_params = {}
    url = url_for(
        "airflow.chart", chart_id=m.id, iteration_no=m.iteration_no, **default_params
    )
...
attr_renderer = {
    ...
    "python_callable": lambda x: render(
        wwwutils.get_python_source(x), lexers.PythonLexer
    ),
}
...
def data_profiling_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if current_app.config["LOGIN_DISABLED"] or (
...
class Airflow(BaseView):
    ...
    def chart_data(self):
        ...
        with create_session() as session:
            ...
            db = (
                ...
                .filter_by(conn_id=chart.conn_id)
                .first()
            )
        ...
        payload["sql_html"] = Markup(
            highlight(
                sql, lexers.SqlLexer(), HtmlFormatter(noclasses=True)  # Lexer call
            )
        )
        payload["label"] = label
        ...
        if csv:
            return Response(
                response=df.to_csv(index=False), status=200, mimetype="application/text"
            )

        if not payload["error"] and len(df) == CHART_LIMIT:
            payload["warning"] = (
                "Data has been truncated to {0}" " rows. Expect incomplete results."
            ).format(CHART_LIMIT)
        ...
        if not payload["error"] and len(df) == 0:
            ...
            not payload["error"]
            and chart.sql_layout == "series"
            and chart.chart_type != "datatable"
            and len(df.columns) < 3
        ):
            payload["error"] += "SQL needs to return at least 3 columns. "
        elif (
            not payload["error"]
            and chart.sql_layout == "columns"
    ...
    def chart(self):
        ...
        if not NVd3ChartClass:
            flash(
                "Not supported anymore as the license was incompatible, " "sorry",
                "danger",
            )
    ...
    def dag_stats(self, session=None):
        ...
        qry = session.query(ds.dag_id, ds.state, ds.count)
    ...
    def task_stats(self, session=None):
        ...
        RunningTI = session.query(
            TI.dag_id.label("dag_id"), TI.state.label("state")
        ).join(
            RunningDagRun,
            and_(
        ...
        qry = session.query(
            UnionTI.c.dag_id, UnionTI.c.state, sqla.func.count()
    ...
    def dag_details(self, session=None):
        ...
        return self.render(
            "airflow/dag_details.html", dag=dag, title=title, states=states, State=State
        )

    @current_app.errorhandler(404)
    def circles(self):
        return render_template("airflow/circles.html", hostname=get_hostname()), 404
    ...
    def task(self):
        ...
        for attr_name in dir(task):
            if not attr_name.startswith("_"):
                ...
                if type(attr) != type(self.task) and attr_name not in attr_renderer:
        ...
        no_failed_deps_result = [
            ...
            If this task instance does not start soon please contact your Airflow """
                    """administrator for assistance.""".format(
    ...
    def delete(self):
        ...
        flash(
            "Deleting DAG with id {}. May take a couple minutes to fully"
    ...
    def _clear_dag_tis(
        self, dag, start_date, end_date, origin, recursive=False, confirmed=False
    ):
        if confirmed:
            count = dag.clear(
                start_date=start_date, end_date=end_date, include_subdags=recursive
            )
    ...
    def clear(self):
        ...
        return self._clear_dag_tis(
            dag, start_date, end_date, origin, recursive=recursive, confirmed=confirmed
        )
    ...
    def dagrun_clear(self):
        ...
        return self._clear_dag_tis(
            dag, start_date, end_date, origin, recursive=True, confirmed=confirmed
        )
    ...
    def _mark_dagrun_state_as_failed(self, dag_id, execution_date, confirmed, origin):
        ...
        if confirmed:
            ...
            response = self.render(
                "airflow/confirm.html",
                message=(
                    "Here's the list of task instances you are "
    ...
    def dagrun_failed(self):
        ...
        confirmed = request.args.get("confirmed") == "true"
        origin = request.args.get("origin")
        return self._mark_dagrun_state_as_failed(
            dag_id, execution_date, confirmed, origin
        )
    ...
    def dagrun_success(self):
        ...
        confirmed = request.args.get("confirmed") == "true"
        origin = request.args.get("origin")
        return self._mark_dagrun_state_as_success(
            dag_id, execution_date, confirmed, origin
        )
    ...
    def tree(self, session=None):
        ...
        if root:
            dag = dag.sub_dag(
                task_regex=root, include_downstream=False, include_upstream=True
            )
        ...
        tis = dag.get_task_instances(session, start_date=min_date, end_date=base_date)
        task_instances = {}
        for ti in tis:
        ...
        def recurse_nodes(task, visited):
            ...
            def set_duration(tid):
                if (
                    isinstance(tid, dict)
                    and tid.get("state") == State.RUNNING
            ...
            return {
                "name": task.task_id,
                "instances": [
                    set_duration(task_instances.get((task.task_id, d)))
                    or {"execution_date": d.isoformat(), "task_id": task.task_id}
                    for d in dates
                ],
                children_key: children,
        ...
        data = {
            ...
            "instances": [
                dag_runs.get(d) or {"execution_date": d.isoformat()} for d in dates
            ],
        }
        ...
        form = DateTimeWithNumRunsForm(
            data={"base_date": max_date, "num_runs": num_runs}
        )
        return self.render(
            "airflow/tree.html",
            operators=sorted(
                list(set([op.__class__ for op in dag.tasks])), key=lambda x: x.__name__
            ),
            root=root,
            form=form,
            dag=dag,
    ...
    def graph(self, session=None):
        ...
        if root:
            dag = dag.sub_dag(
                task_regex=root, include_upstream=True, include_downstream=False
            )
        ...
        def get_upstream(task):
            for t in task.upstream_list:
                edge = {"u": t.task_id, "v": task.task_id}
        ...
        tasks = {
            t.task_id: {"dag_id": t.dag_id, "task_type": t.task_type} for t in dag.tasks
        }
        if not tasks:
            flash("No tasks found", "error")
        session.commit()
        doc_md = (
            markdown.markdown(dag.doc_md)
        ...
        for task in dag.tasks:
            if x[task.task_id]:
                ...
                cum_chart.add_serie(
                    name=task.task_id,
                    x=x[task.task_id],
                    y=scale_time_units(cum_y[task.task_id], cum_y_unit),
                )
    ...
    def tries(self, session=None):
        ...
        for task in dag.tasks:
            y = []
            x = []
            for ti in task.get_task_instances(
    ...
    def gantt(self, session=None):
        ...
        tis = [
            ti for ti in dag.get_task_instances(session, dttm, dttm) if ti.start_date
        ]
    ...
    def variables(self, form):
        try:
            if request.method == "POST":
                ...
                return self.render("airflow/variables/{}.html".format(form))
        except Exception:
            # prevent XSS
            form = escape(form)
            return (
                ("Error: form airflow/variables/{}.html " "not found.").format(form),
                404,
            )
    ...
    def index(self, session=None):
        ...
        if do_filter and owner_mode == "ldapgroup":
            sql_query = sql_query.filter(
                ~DM.is_subdag, DM.is_active, DM.owners.in_(current_user.ldap_groups)
            )
        elif do_filter and owner_mode == "user":
            sql_query = sql_query.filter(
                ~DM.is_subdag, DM.is_active, DM.owners == current_user.user.username
            )
        else:
            sql_query = sql_query.filter(~DM.is_subdag, DM.is_active)
        ...
        if hide_paused:
            unfiltered_webserver_dags = [
                dag
                for dag in dagbag.dags.values()
                if not dag.parent_dag and not dag.is_paused
            ]
        ...
        if do_filter and owner_mode == "ldapgroup":
            ...
            webserver_dags = {dag.dag_id: dag for dag in unfiltered_webserver_dags}
        ...
        if arg_search_query:
            ...
            webserver_dags_filtered = {
                ...
                if (
                    lower_search_query in dag_id.lower()
                    or lower_search_query in dag.owner.lower()
                )
            }
            ...
            all_dag_ids = set(
                [
                    ...
                    if lower_search_query in dag.dag_id.lower()
                    or lower_search_query in dag.owners.lower()
                ]
...
class QueryView(wwwutils.DataProfilingMixin, BaseView):
    ...
    def query(self, session=None):
        dbs = session.query(models.Connection).order_by(models.Connection.conn_id).all()
        session.expunge_all()
        ...
        data = {"conn_id": conn_id_str, "sql": sql}
        results = None
        has_data = False
        error = False
        if conn_id_str:
            ...
            try:
                df = hook.get_pandas_df(
                    wwwutils.limit_sql(sql, QUERY_LIMIT, conn_type=db.conn_type)
                )
...
class PoolModelView(wwwutils.SuperUserMixin, AirflowModelView):
    ...
    named_filter_urls = True
    form_args = {"pool": {"validators": [validators.DataRequired()]}}


class SlaMissModelView(wwwutils.SuperUserMixin, ModelViewOnly):
    verbose_name_plural = "SLA misses"
    verbose_name = "SLA miss"
    column_list = ("dag_id", "task_id", "execution_date", "email_sent", "timestamp")
    column_formatters = dict(
    ...
    column_filters = ("dag_id", "task_id", "email_sent", "timestamp", "execution_date")
...
def _connection_ids(session=None):
    return [
        (c.conn_id, c.conn_id)
        for c in (
            session.query(models.Connection.conn_id).group_by(models.Connection.conn_id)
        )
    ]
...
class ChartModelView(wwwutils.DataProfilingMixin, AirflowModelView):
    ...
    column_list = ("label", "conn_id", "chart_type", "owner", "last_modified")
    ...
    column_descriptions = {
        ...
        "show_datatable": "Whether to display an interactive data table under the chart.",
        "default_params": (
...
class KnownEventView(wwwutils.DataProfilingMixin, AirflowModelView):
    ...
    form_args = {
        "label": {"validators": [validators.DataRequired()]},
        "event_type": {"validators": [validators.DataRequired()]},
        "start_date": {
            "validators": [validators.DataRequired()],
            "filters": [parse_datetime_f],
        },
        "end_date": {
            "validators": [
        ...
        "reported_by": {"validators": [validators.DataRequired()]},
    }
    column_list = ("label", "event_type", "start_date", "end_date", "reported_by")
...
class VariableView(wwwutils.DataProfilingMixin, AirflowModelView):
    ...
    form_columns = ("key", "val")
    column_list = ("key", "val", "is_encrypted")
    column_filters = ("key", "val")
    column_searchable_list = ("key", "val", "is_encrypted")
    column_default_sort = ("key", False)
    form_widget_args = {"is_encrypted": {"disabled": True}, "val": {"rows": 20}}
    form_args = {"key": {"validators": {validators.DataRequired()}}}
    column_sortable_list = ("key", "val", "is_encrypted")
    column_formatters = {"val": hidden_field_formatter}
...
class XComView(wwwutils.SuperUserMixin, AirflowModelView):
    ...
    form_columns = ("key", "value", "execution_date", "task_id", "dag_id")

    form_extra_fields = {"value": StringField("Value")}

    form_args = {"execution_date": {"filters": [parse_datetime_f]}}
...
class DagRunModelView(ModelViewOnly):
    ...
    form_choices = {
        "state": [("success", "success"), ("running", "running"), ("failed", "failed")]
    }
    form_args = dict(dag_id=dict(validators=[validators.DataRequired()]))
    column_list = ("state", "dag_id", "execution_date", "run_id", "external_trigger")
    ...
    def action_new_delete(self, ids, session=None):
        deleted = set(
            session.query(models.DagRun).filter(models.DagRun.id.in_(ids)).all()
        )
        session.query(models.DagRun).filter(models.DagRun.id.in_(ids)).delete(
...
class ConnectionModelView(wwwutils.SuperUserMixin, AirflowModelView):
    ...
    form_choices = {"conn_type": models.Connection._types}
...
class VersionView(wwwutils.SuperUserMixin, BaseView):
    ...
    def version(self):
        ...
        try:
            with open(
                os.path.join(*[settings.AIRFLOW_HOME, "airflow", "git_version"])
            ) as f:
...
class DagModelView(wwwutils.SuperUserMixin, ModelView):
    ...
    column_formatters = dict(dag_id=dag_link)
    can_delete = False
    can_create = False
    page_size = PAGE_SIZE
