...
def setup_locations(process, pid=None, stdout=None, stderr=None, log=None):
    if not stderr:
        stderr = os.path.join(
            os.path.expanduser(settings.AIRFLOW_HOME), "airflow-{}.err".format(process)
        )
    if not stdout:
        stdout = os.path.join(
            os.path.expanduser(settings.AIRFLOW_HOME), "airflow-{}.out".format(process)
        )
    if not log:
        log = os.path.join(
            os.path.expanduser(settings.AIRFLOW_HOME), "airflow-{}.log".format(process)
        )
    if not pid:
        pid = os.path.join(
            os.path.expanduser(settings.AIRFLOW_HOME), "airflow-{}.pid".format(process)
        )
...
def upgradedb(args):  # noqa
    ...
    if not ds_rows:
        qry = session.query(DagRun.dag_id, DagRun.state, func.count("*")).group_by(
...
def flower(args):
    ...
    if args.daemon:
        ...
        ctx = daemon.DaemonContext(
            pidfile=TimeoutPIDLockFile(pid, -1), stdout=stdout, stderr=stderr
        )
...
class CLIFactory(object):
    args = {
        ...
        "username": Arg(("-u", "--username"), help="Username of the user", type=str),
        # list_dag_runs
        "no_backfill": Arg(
            ("--no_backfill",),
        ...
        "firstname": Arg(
            ("-f", "--firstname"), help="First name of the user", type=str
        ),
        "lastname": Arg(("-l", "--lastname"), help="Last name of the user", type=str),
        "email": Arg(("-e", "--email"), help="Email of the user", type=str),
        "password": Arg(("-p", "--password"), help="Password of the user", type=str),
    ...
    subparsers = (
        ...
        {"func": pause, "help": "Pause a DAG", "args": ("dag_id", "subdir")},
        {"func": unpause, "help": "Resume a paused DAG", "args": ("dag_id", "subdir")},
        {
            "func": trigger_dag,
        ...
        {"func": initdb, "help": "Initialize the metadata database", "args": tuple()},
        {"func": list_dags, "help": "List all the DAGs", "args": ("subdir", "report")},
        {
            "func": dag_state,
        ...
        {"func": serve_logs, "help": "Serve logs generate by worker", "args": tuple()},
        {
            "func": test,
            "help": (
        ...
        {"func": version, "help": "Show the version", "args": tuple()},
        {
            "func": connections,
        ...
        {"func": list_users, "help": "List accounts for the Web UI", "args": tuple()},
        {
            "func": sync_perm,
