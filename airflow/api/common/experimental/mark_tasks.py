...
def set_state(
    ...
    qry_dag = (
        ...
        .filter(or_(TI.state.is_(None), TI.state != state))
    )
    ...
    if len(sub_dag_ids) > 0:
        qry_sub_dag = (
            session.query(TI)
            .filter(TI.dag_id.in_(sub_dag_ids), TI.execution_date.in_(confirmed_dates))
            .filter(or_(TI.state.is_(None), TI.state != state))
        )
...
def _set_dag_run_state(dag_id, execution_date, state, session=None):
    ...
    dr = (
        session.query(DR)
        .filter(DR.dag_id == dag_id, DR.execution_date == execution_date)
        .one()
    )
    dr.state = state
...
def set_dag_run_state_to_success(dag, execution_date, commit=False, session=None):
    ...
    for task in dag.tasks:
        task.dag = dag
        new_state = set_state(
            task=task, execution_date=execution_date, state=State.SUCCESS, commit=commit
        )
...
def set_dag_run_state_to_failed(dag, execution_date, commit=False, session=None):
    ...
    for task in dag.tasks:
        ...
        task.dag = dag
        new_state = set_state(
            task=task, execution_date=execution_date, state=State.FAILED, commit=commit
        )
        res.extend(new_state)
